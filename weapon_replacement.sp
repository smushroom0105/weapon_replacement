...
public void OnPluginStart()
{
    ...
    for (int i = 0; i <= MAXPLAYERS; i++)
        gQueue[i] = new ArrayList(ByteCountToCells(64));
...
void InitWeaponData()
{
    ...
    gPrices.SetValue(W_P2000, 200);
    gPrices.SetValue(W_USP,   200);
    gPrices.SetValue(W_M4A4,  3100);
    gPrices.SetValue(W_M4A1,  2900);
    gPrices.SetValue(W_MP7,   1500);
    gPrices.SetValue(W_MP5,   1500);
    gPrices.SetValue(W_DEAGLE,700);
    gPrices.SetValue(W_R8,    600);
    gPrices.SetValue(W_FIVE7, 500);
    gPrices.SetValue(W_TEC9,  500);
    gPrices.SetValue(W_CZ75,  500);
...
{
    if (!IsRealClient(client)) { ReplyToCommand(client, "[SM] 你必须在游戏中。"); return Plugin_Handled; }
...
public Action EventSpawn(Event e, const char[] n, bool nb)
{
    ...
    if (!IsRealClient(client) || !IsPlayerAlive(client)) return Plugin_Continue;
...
{
    int client = GetClientOfUserId(e.GetInt("userid"));
    if (IsRealClient(client)) gQueue[client].Clear();
    return Plugin_Continue;
}
...
public Action TimerInitial(Handle t, any userid)
{
    ...
    if (IsRealClient(client) && IsPlayerAlive(client) && gPref[client].loaded)
        CheckReplace(client);
    return Plugin_Stop;
}
...
void CheckReplace(int client)
{
    ...
    if (team == CS_TEAM_CT)
    {
        if (gPref[client].m4a4 && FindWeapon(client, W_M4A1) == -1) Replace(client, W_M4A4, W_M4A1, false);
        if (gPref[client].p2000 && FindWeapon(client, W_USP) == -1) Replace(client, W_P2000, W_USP, false);
        if (gPref[client].pistols && FindWeapon(client, W_CZ75) == -1) Replace(client, W_FIVE7, W_CZ75, false);
    }
    else if (team == CS_TEAM_T && gPref[client].pistols)
    {
        if (FindWeapon(client, W_CZ75) == -1) Replace(client, W_TEC9, W_CZ75, false);
    }

    if (gPref[client].mp7 && FindWeapon(client, W_MP5) == -1) Replace(client, W_MP7, W_MP5, false);
    if (gPref[client].deagle && FindWeapon(client, W_R8) == -1) Replace(client, W_DEAGLE, W_R8, false);
}

public Action EventPurchase(Event e, const char[] n, bool nb)
{
    int client = GetClientOfUserId(e.GetInt("userid"));
    if (!IsRealClient(client) || !IsPlayerAlive(client) || !gPref[client].loaded) return Plugin_Continue;
...
public Action TimerReplace(Handle t, DataPack dp)
{
    ...
    char info[64]; dp.ReadString(info, sizeof(info));
    bool refund = dp.ReadCell();
    delete dp;

    if (!IsRealClient(client) || !IsPlayerAlive(client)) return Plugin_Stop;

    int idx = gQueue[client].FindString(info);
    if (idx == -1) return Plugin_Stop;
    gQueue[client].Erase(idx);

    char parts[2][32];
    if (ExplodeString(info, ";", parts, 2, 32) != 2) return Plugin_Stop;

    char oldW[32], newW[32];
    strcopy(oldW, sizeof(oldW), parts[0]);
    strcopy(newW, sizeof(newW), parts[1]);
